...
class Dense(Layer):  # epsilon
    ...
    def inputOK(self):
        if self.inputShape is not None:
            ...
            self.weights = np.random.standard_normal((self.neurons, self.inputShape[0]))/ np.sqrt(self.lastDim/2)
            self.bias = np.random.standard_normal((self.neurons, 1)) / np.sqrt(self.lastDim/2)
    ...
    def feedBackward(self, feedback: np.array, clipval):
        ...
        self.loss_div_weights = np.array(np.matmul(self.input, self.loss_div_bias)).T
        self.backPropagationW = self.backPropagationW + np.array(clipval(self.loss_div_weights))
        if self.biasUsed:
            self.backPropagationB = self.backPropagationB + np.array(clipval(self.loss_div_bias.T))
...
class Convolution2D(Layer):
    ...
    def feedBackward(self, feedback: np.array, clipval):
        ...
        self.backPropagationW = self.backPropagationW + clipval(self.loss_div_weights)
        self.backPropagationB = self.backPropagationB + clipval(self.loss_div_bias)
